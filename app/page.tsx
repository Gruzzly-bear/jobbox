          ...
          {docs.length > 0 ? docs.map((doc) => (
            <div key={doc.id} className="flex items-center justify-between p-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); downloadDoc(doc.file_path, doc.tag); }} 
                  className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition-all shadow-sm active:scale-95" 
                  title="Download"
                >
                ...
                <button 
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); deleteDoc(doc.id, doc.file_path); }} 
                  className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-400 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all shadow-sm active:scale-95" 
                  title="Delete"
                >
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center justify-between font-black tracking-tight text-lg ${!loading ? 'pl-2' : ''}`}>
        <span className="truncate pr-2">{loading === tag ? 'UPLOADING...' : label.toUpperCase()}</span>
        {!loading && (
          <svg className="w-5 h-5 shrink-0 text-slate-300 group-hover:text-slate-900 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
